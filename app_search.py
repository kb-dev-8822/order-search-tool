...
def load_data():
    ...
    # --- השינוי הגדול: טעינה מתוך Secrets במקום מקובץ מקומי ---
    # אנחנו בודקים אם המפתח קיים בכספת של סטרימליט
...
with col_radio:
    search_type = st.radio("חפש לפי:", ("טלפון", "מספר הזמנה"), horizontal=True)
    ...
    if not filtered_df.empty:
        ...
        if df.shape[1] > 9:
            try:
                filtered_df["temp_date"] = pd.to_datetime(
                    filtered_df.iloc[:, 9], dayfirst=True, errors="coerce"
                )
        ...
        for index, row in filtered_df.iterrows():
            try:
                ...
                name = row.iloc[3]
                addr_parts = [
                    str(row.iloc[i]) for i in [4, 5, 6] if pd.notna(row.iloc[i])
                ]
                address = " ".join(addr_parts)
                phone_raw = row.iloc[7]
                phone_display = "0" + str(phone_raw) if phone_raw else ""
                tracking = row.iloc[8]
                if pd.isna(tracking) or str(tracking).strip() == "":
                    tracking = "התקנה"
                date_val = row.iloc[9]
                ...
                formatted_text = (
                    f'פרטי הזמנה: מספר הזמנה: {order_num}, כמות: {qty}, מק"ט: {sku}, '
